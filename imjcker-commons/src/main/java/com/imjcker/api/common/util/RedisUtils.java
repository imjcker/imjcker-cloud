...
/**
 * <p>Title: RedisUtils.java
 * <p>Description: Redis访问工具类
...
public class RedisUtils {
    ...
    /*static {
        try {
            String redisPropertiesPath = new StringBuffer(GlobalVariable.environment).append("/redis.properties").toString();
    ...
    private static void initialPool(){
        ...
        try {
            if(jedisPool == null) {
                ...
                jedisPool = new JedisPool(config, "101.200.135.111", 6379, 10000, "lemonkz9*l");
                LOGGER.debug("创建Redis Pool成功.");
            }
        } catch (Exception ex) {
    ...
    private static void setShutdownWork() {
        try{
            ...
            LOGGER.debug("设置系统停止时关闭Redis Pool的任务成功.");
        } catch (Exception ex) {
    ...
    public static Jedis getJedis() {
        ...
        try {
            RedisTemplate re = SpringUtils.getBean("redisTemplate", RedisTemplate.class);
            Object nativeConnection = re.getConnectionFactory().getConnection().getNativeConnection();
